...
export const resolveProduct = tool({
  name: "resolveProduct",
  description:
    "Resolve a natural-language product reference to canonical SKU(s) in the caller's organization. Returns ranked candidates with similarity scores and an `ambiguous` flag. Call this whenever the user references a product by name/description instead of a canonical SKU (regex: ^[A-Z]{2,5}-\\d{3,}$). If `ambiguous` is true, ask the user to disambiguate before calling any SKU-keyed tool. This is a CHEAP read tool — call it freely. Never skip it to 'save a step'; skipping causes the next tool to fail with sku_not_resolved.",
  parameters: z.object({
